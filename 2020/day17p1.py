...
class Space(object):
    ...
    def neighbors(self, point):
        ...
        for z in (-1, 0, 1):
            for y in (-1, 0, 1):
                for x in (-1, 0, 1):
                    if Point(x, y, z) == Point(0, 0, 0):
                        continue
                    else:
                        t = Point(point.x+x, point.y+y, point.z+z)
                        res.append(self[t])
        return res
    ...
    def cycle(self):
        ...
        if self.cycle == 0:
            ...
            self.min = Point(self.min.x-1, self.min.y-1, self.min.z-1)
            self.max = Point(self.max.x+1, self.max.y+1, self.max.z+1)
